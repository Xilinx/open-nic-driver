...
/* always undefine this macro for SN1000 */
#undef XRT_SUPPORT
#undef XMC_CHRDEV
#undef XMC_QSFP
#undef XMC_MAILBOX
...
#define	XMC_FEATURE_REG			0x10    /* not in sn1000 */
#define	XMC_SENSOR_REG			0x14
#define	XMC_CONTROL_REG			0x18
#define	XMC_STOP_CONFIRM_REG	0x1C    /* not in sn1000 */
...
#define	XMC_HEARTBEAT_REG       0x2FC    /* not in sn1000 */
#define	XMC_HOST_MSG_OFFSET_REG	0x300
#define	XMC_HOST_MSG_ERROR_REG	0x304
#define	XMC_HOST_MSG_HEADER_REG	0x308    /* not in sn1000 */
#define	XMC_STATUS2_REG			0x30C
#define	XMC_HEARTBEAT_ERR_CODE_REG	0x310    /* not in sn1000 */
	...
	void *buf)
{
    ...
    #else
	if (XMC_PRIVILEGED(xmc) && !autonomous_xmc(pdev)) {
	#endif
...
{
	u32 base, max, cntrl;
	u32 c_12v_pex = 0, c_3v3_pex = 0, c_12v_aux = 0, vccint_c = 0,
...
{
	u32 fpga_temp, base, max, cntrl, vccint_temp = 0;
	...
	struct device_attribute *da, char *buf)
{
	...
	struct platform_device *pdev = to_platform_device(dev);
	int index = to_sensor_dev_attr(da)->index;
	val = xmc_get_power(pdev, HWMON_INDEX2VAL_KIND(index));
...
HWMON_VOLT_CURR_SYSFS_NODE(in, 0, " 12V PEX", VOL_12V_PEX); /* sn1000 */
HWMON_VOLT_CURR_SYSFS_NODE(in, 1, " 12V AUX", VOL_12V_AUX); /* sn1000 */
HWMON_VOLT_CURR_SYSFS_NODE(in, 2, " 3V3 PEX", VOL_3V3_PEX); /* sn1000 */
#ifdef XMC_XRT
...
#endif
HWMON_VOLT_CURR_SYSFS_NODE(in, 7, " 1V8 TOP", VOL_1V8); /* sn1000 */
#ifdef XMC_XRT
HWMON_VOLT_CURR_SYSFS_NODE(in, 8, " 12V SW", VOL_12V_SW);
#endif
HWMON_VOLT_CURR_SYSFS_NODE(in, 9, " VCC INT", VOL_VCC_INT); /* sn1000 */
#ifdef XMC_XRT
...
#endif
HWMON_VOLT_CURR_SYSFS_NODE(in, 15, " VCC 3V3", VOL_VCC_3V3); /* sn1000 */
#ifdef XMC_XRT
...
#endif
HWMON_VOLT_CURR_SYSFS_NODE(curr, 1, " 12V PEX Current", CUR_12V_PEX); /* sn1000 */
HWMON_VOLT_CURR_SYSFS_NODE(curr, 2, " 12V AUX Current", CUR_12V_AUX); /* sn1000 */
HWMON_VOLT_CURR_SYSFS_NODE(curr, 3, " VCC INT Current", CUR_VCC_INT); /* sn1000 */
HWMON_VOLT_CURR_SYSFS_NODE(curr, 4, " 3V3 PEX Current", CUR_3V3_PEX); /* sn1000 */
#ifdef XMC_XRT
...
#endif
HWMON_TEMPERATURE_SYSFS_NODE(1, " PCB TOP FRONT", SE98_TEMP0); /* sn1000 */
HWMON_TEMPERATURE_SYSFS_NODE(2, " PCB TOP REAR", SE98_TEMP1); /* sn1000 */
#ifdef XMC_XRT
HWMON_TEMPERATURE_SYSFS_NODE(3, " PCB BTM FRONT", SE98_TEMP2);
#endif
HWMON_TEMPERATURE_SYSFS_NODE(4, " FPGA TEMP", FPGA_TEMP); /* sn1000 */
#ifdef XMC_XRT
...
#endif
HWMON_TEMPERATURE_SYSFS_NODE(11, " QSPF 0", CAGE_TEMP0); /* sn1000 */
HWMON_TEMPERATURE_SYSFS_NODE(12, " QSPF 1", CAGE_TEMP1); /* sn1000 */
#ifdef XMC_XRT
...
static int mgmt_sysfs_create_xmc(struct platform_device *pdev)
{
	...
	if (!xmc->enabled)
	{
		xocl_err(&pdev->dev, "xmc not enabled \r\n");
		return 0;
	}
	...
	#else
	xmc->hwmon_dev = hwmon_device_register(&pdev->dev);
	#endif
...
#ifdef XMC_XRT
static void xmc_sensor_status_check(struct platform_device *pdev)
{
	struct xocl_xmc *xmc = platform_get_drvdata(pdev);
...
static struct xocl_mb_funcs xmc_ops = {
    ...
    #ifdef XMC_XRT
	.sensor_status= xmc_sensor_status_check,
...
int xmc_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < NUM_IOADDR; i++) {
		...
		if (res) {
			printk(KERN_ERR "  IO start: 0x%llx, end: 0x%llx",
				res->start, res->end);
			...
			/* fall back to legacy */
			printk(KERN_ERR "  ioremap_nocache \r\n");
...
failed:
    xocl_err(&pdev->dev, "xmc_probe failed \r\n");
	xmc_remove(pdev);
	return err;
}
...
MODULE_DEVICE_TABLE(platform, xmc_id_table);
...
static struct platform_device xmcc_device = {
	.name = XOCL_DEVNAME(XOCL_XMC),
	.id = PLATFORM_DEVID_NONE,
};

static const struct of_device_id xmc_of_ids[] = {
	{ .compatible = XOCL_DEVNAME(XOCL_XMC) },
	{ }
};
MODULE_DEVICE_TABLE(of, xmc_of_ids);
...
int xocl_init_xmc(void)
{
	...
	platform_device_register(&xmcc_device);

	err = platform_driver_register(&xmc_driver);
	if (err) {
		platform_device_del(&xmcc_device);
		return err;
	}
...
#ifdef XMC_XRT
module_init(xocl_init_xmc);
module_exit(xocl_fini_xmc);
#endif
...
{
	int ret = 0;
	struct xocl_xmc *xmc=file->private_data = xocl_drvinst_open(inode->i_cdev);
