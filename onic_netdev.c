...
static int onic_rx_poll(struct napi_struct *napi, int budget)
{
    ...
    pidx_val = qdma_read_reg((struct qdma_dev *)priv->hw.qdma, offset);
    ...
    cmpl_stat_ptr = cmpl_ring->desc +
                    QDMA_C2H_CMPL_SIZE * (cmpl_ring->count-1);
    ...
    if (debug) netdev_info (q->netdev, "\n rx_poll: pidx is %u, cmpl_stat_pidx %u, color_cmpl_stat %u, cmpl_ring next_to_clean %u, cmpl_stat_cidx %u, intr_state %u, cmpl_ring->count %u",
                 pidx_val, cmpl_stat.pidx, color_stat, cmpl_ring->next_to_clean, cmpl_stat.cidx, cmpl_stat.intr_state, cmpl_ring->count);

    if (debug) netdev_info(q->netdev, "c2h_cmpl pkt_id %u, pkt_len %u, error %u, color %u cmpl_ring->color:%u", cmpl.pkt_id, cmpl.pkt_len, cmpl.err, cmpl.color, cmpl_ring->color);

    /* The earlier behavior was based on checking the color and the below while looping until the color changed.
     * This while loop has since been changed to be based on comparing the PIDX to the CIDX (cmpl_ring->next_to_clean).
     *
     * Old description:
    ...
    if (cmpl.color != cmpl_ring->color) {
        if (debug) netdev_info(q->netdev, "color mismatch1: cmpl.color %u, cmpl_ring->color %u  cmpl_stat_color %u", cmpl.color, cmpl_ring->color, color_stat);
    }

    if (cmpl.err == 1) {
        if (debug) netdev_info(q->netdev, "completion error detected in cmpl entry!");
    ...
    while ((cmpl_ring->next_to_clean != cmpl_stat.pidx)) {
        ...
        if (debug) netdev_info(q->netdev, "desc_ring %u next_to_use:%u next_to_clean:%u", onic_ring_get_real_count(desc_ring), desc_ring->next_to_use, desc_ring->next_to_clean);
        ...
        if (debug) netdev_info(q->netdev, "cmpl_ring %u next_to_use:%u next_to_clean:%u, flipped:%s", onic_ring_get_real_count(cmpl_ring), cmpl_ring->next_to_use, cmpl_ring->next_to_clean, flipped?"true":"false");
        if(onic_ring_full(cmpl_ring)) {
            netdev_dbg(q->netdev, "cmpl_ring full");
        }

        if (cmpl_ring->next_to_clean == 0) {
            if (debug) netdev_info(q->netdev, "part 1. cmpl_ring->next_to_clean=%u color *** old fliping *** color[%u]", cmpl_ring->next_to_clean, cmpl_ring->color);
        ...
        qdma_unpack_c2h_cmpl(&cmpl, cmpl_ptr);
        if (debug) netdev_info(q->netdev, "c2h_cmpl(b) pkt_id %u, pkt_len %u, error %u, color %u", cmpl.pkt_id, cmpl.pkt_len, cmpl.err, cmpl.color);

    }

    if (debug) netdev_info(q->netdev, "outside while loop, pidx %u, napi_complete work %u, budget %u, rval %s", cmpl_stat.pidx, work, budget, napi_cmpl_rval? "true" : "false");
    ...
    if (debug) netdev_info(q->netdev, "onic_set_completion_tail called");
...
static int onic_init_rx_queue(struct onic_private *priv, u16 qid)
{
    ...
    param.vid = vid;
    if (debug) netdev_info(dev, "bufsz_idx %u, desc_rngcnt_idx %u, cmpl_rngcnt_idx %u, desc_dma_addr 0x%p, cmpl_dma_addr 0x%p, vid %d", bufsz_idx, desc_rngcnt_idx, cmpl_rngcnt_idx,
                           (void *) q->desc_ring.dma_addr, (void *) q->cmpl_ring.dma_addr, vid);
...
netdev_tx_t onic_xmit_frame(struct sk_buff *skb, struct net_device *dev)
{
    ...
    if (unlikely(dma_mapping_error(&priv->pdev->dev, dma_addr))) {
        netdev_err(dev, "dma map error, skb = %p, dma_addr = %p",
                   skb, (void *) dma_addr);
        return NETDEV_TX_BUSY;
    }
